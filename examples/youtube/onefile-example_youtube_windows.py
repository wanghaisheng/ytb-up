...
channelconfig = load_config_from_json(os.path.join(os.getcwd(),'examples/youtube/youtube-account-one.json'))
...
def saveasprivatedraft(video_settings):
    video_settings['publish_policy']=VideoSetting.PublishPolicyOptions.PRIVATE
    # Loading configuration and validating
    try:
        uploader.upload_video(video_settings)
    except ValueError as e:
...
def scheduletopublish_tomorrow(video_settings):
    ...
    video_settings['publish_policy']=VideoSetting.PublishPolicyOptions.SCHEDULE
    # Loading configuration and validating
    try:
        uploader.upload_video(video_settings)
    except ValueError as e:
...
video_settings = load_config_from_json(os.path.join(os.getcwd(),'tests/horizon/youtube-test-video.json'))
