...
class YoutubeUpload:
    ...
    def upload_video(self, video_settings) -> Tuple[bool, Optional[str]]:
        ...
        # self.page.wait.title_change('hao123')  # 等待title变化出现目标文本

        # Check login status
        logger.info("start to check login status")
        if not self.check_login_status():
            logger.info("start to use cookie login")

            self.cookielogin()

        self.navigate_to_upload_studiopage()
        self.perform_upload(videosettings)

        video_id = self.get_video_id()
        self.finalize_upload(self)
    ...
    def perform_upload(
        ...
        self.logger.debug(f"Uploading video from {videosettings.get('video_local_path')}")
        self.set_videofile(self, INPUT_FILE_VIDEO, videosettings.get('video_local_path'))

        # Set title and description
        self.set_title(self, videosettings.get('video_title'))
        self.set_description(self, videosettings.get('video_description'))

        # Add additional upload logic here (tags, thumbnail, etc.)
        # self.add_additional_video_info(self, **kwargs)
    ...
    def navigate_to_upload_studiopage(self):
        ...
        isenglish=self.check_is_english()
        if not isenglish:
    ...
    def change_to_english(self):
        self.page.ele('#end').ele("tag:button@aria-label=Settings").click()


        self.page.ele('tag:ytd-toggle-theme-compact-link-renderer').next().ele("#subtitle").click()
        self.page.ele('#items').ele('text=English (US)').click()
        self.page.ele('#end').ele("tag:button@aria-label=Settings").click()
        
        logger.info('end to change home page lang to english')

    def check_is_english(self):
            
        self.page.ele('#end').ele("tag:button@aria-label=Settings").click()
        # print(self.page.ele('tag:ytd-app').texts())
        # print(self.page.ele('tag:ytd-app').ele("tag:tp-yt-iron-dropdown").texts())
        print(self.page.ele('tag:ytd-toggle-theme-compact-link-renderer').next().texts())
        # print(self.page.ele('xpath://*[@id="items"]/ytd-compact-link-renderer[2]').text)
        currentlang=self.page.ele('tag:ytd-toggle-theme-compact-link-renderer').next().ele('#subtitle').text
        logger.info(f'current lang is:{currentlang}')
        if not currentlang=='English':
            return False
        else:
            return True
